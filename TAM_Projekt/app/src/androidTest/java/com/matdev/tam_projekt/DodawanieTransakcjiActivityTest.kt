...
class DodawanieTransakcjiActivityTest {
    ...
    fun uruchomienie_ekranu_powinno_pokazac_odpowiednie_widoki() {
        ...
        onView(withId(R.id.zmianaTransakcjiButton)).check(matches(isDisplayed()))
        onView(withId(R.id.zmianaTransakcjiButton)).check(matches(isDisplayed()))
    ...
    fun klikniecie_w_przycisk_bez_danych_powinno_pokazac_blad() {
        ...
        //onView(withId(R.id.)).check(matches(hasErrorText("Proszę podać prawidłową etykiete transakcji!")))
    }
}